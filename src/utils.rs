...
pub fn str2bool(v: &str) -> Result<bool> {
    match v.to_lowercase().as_str() {
        ...
        _ => Err("Boolean value expected.".to_string().into()),
    }
}

pub fn memory_cost_qwen(config: &Qwen2MoeConfig, memory_budget_gb: usize) -> Result<(HashMap<usize, usize>, HashMap<usize, usize>)> {
    let nvml = Nvml::init()?;
    let device_index: u32 = config.device.strip_prefix("cuda:")
        .ok_or_else(|| anyhow::anyhow!("Invalid device string"))?
        .parse()
...
pub fn pack_3bit_32(w: &Tensor) -> candle_core::Result<Tensor> {
    ...
    let mut padded = Tensor::zeros((padded_n, d), DType::I32, device)?;
    padded.i(0..n)?.copy_(&w.to_dtype(DType::I32)?)?;
