...
pub fn dequantize(quantized_data: &HashMap<String, Tensor>) -> Result<Tensor> {
    let w_q = quantized_data
        .get("W_q")
        .ok_or(candle_core::Error::Msg("Missing key: W_q".into()))?;

    let mut scale = quantized_data
        .get("scale")
        .ok_or(candle_core::Error::Msg("Missing key: scale".into()))?
        .clone();
    if scale.dtype() != DType::BF16 {
        scale = scale.to_dtype(DType::BF16)?;
    }

    let mut zero = quantized_data
        .get("zero")
        .ok_or(candle_core::Error::Msg("Missing key: zero".into()))?
        .clone();
    if zero.dtype() != DType::BF16 {
        zero = zero.to_dtype(DType::BF16)?;
    }

    let mut shape = quantized_data
        .get("shape")
        .ok_or(candle_core::Error::Msg("Missing key: shape".into()))?
        .clone();
    ...
    let mut nbits = quantized_data
        .get("nbits")
        .ok_or(candle_core::Error::Msg("Missing key: nbits".into()))?
        .clone();
