use candle_core::{DType, Device, Result, Tensor, Error};
...
impl Embedding {
    ...
    pub fn new(vocab_size: usize, hidden_size: usize, padding_idx: Option<usize>, device: &Device) -> Result<Self> {
        let weight = Tensor::zeros((vocab_size, hidden_size), DType::F32, device)?;
        Ok(Self { weight, padding_idx })
    }
    ...
    pub fn from_weight(&self, mut weight: Tensor, padding_idx: Option<usize>) -> Result<Self> {
        ...
        if let Some(pidx) = padding_idx {
            ...
            let mask = mask.to_dtype(weight.dtype())?.broadcast_as(weight.shape())?;
            weight = (&weight * &mask)?;
        }
    ...
    pub fn forward(&self, input_ids: &Tensor) -> Result<Tensor> {
        ...
        let embeddings = embeddings.reshape(&[original_dims[0], original_dims[1], self.weight.dims()[1]])?;
        ...
        if let Some(pidx) = self.padding_idx {
            ...
            let result = mask.reshape(&[original_dims[0], original_dims[1], 1])?
